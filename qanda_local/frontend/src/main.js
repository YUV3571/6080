...
function getRoot(){
  return document.getElementById('app-root')||document.querySelector('main');
}
...
function renderLoginForm(card){
  ...
  const submitBtn=el('button',{id:'login-submit',className:'btn-primary',textContent:'Sign In',style:{width:'100%',marginBottom:'16px'}});
  card.appendChild(submitBtn);

  const regRow=el('div',{style:{textAlign:'center',fontSize:'0.875rem',color:'var(--text-muted)'}});
...
function renderRegisterForm(card){
  ...
  const submitBtn=el('button',{id:'register-submit',className:'btn-primary',textContent:'Create Account',style:{width:'100%',marginBottom:'16px'}});
  card.appendChild(submitBtn);

  const loginRow=el('div',{style:{textAlign:'center',fontSize:'0.875rem',color:'var(--text-muted)'}});
...
function updateHeader(loggedIn){
  ...
  const logo=el('div',{className:'header-logo',style:{cursor:loggedIn?'pointer':'default'},title:loggedIn?'Go to homepage':''});
  logo.appendChild(document.createTextNode('Q'));
  logo.appendChild(el('span',{textContent:'&'}));
  logo.appendChild(document.createTextNode('A'));
  logo.appendChild(el('span',{style:{color:'var(--text-muted)',fontWeight:'400',fontSize:'1rem',marginLeft:'4px'},textContent:'qanda'}));
  if (loggedIn){
  ...
  if (loggedIn){
    ...
    const avatarImg=el('img',{src:generateAvatar(state.currentUser?state.currentUser.name:'?'),alt:'Profile'});
...
function renderDashboard(){
  ...
  const listHeader=el('div',{className:'thread-list-header',style:{display:'flex',justifyContent:'space-between',alignItems:'center'}});
  ...
  const searchWrapper=el('div',{style:{padding:'8px 12px',borderBottom:'1px solid var(--border)'}});
  ...
  const sortWrapper=el('div',{style:{padding:'6px 12px',borderBottom:'1px solid var(--border)',display:'flex',gap:'6px',alignItems:'center'}});
  sortWrapper.appendChild(el('span',{style:{fontSize:'0.72rem',color:'var(--text-dim)',textTransform:'uppercase',letterSpacing:'0.06em'}},'Sort:'));
  const sortSelect=el('select',{style:{fontSize:'0.78rem',padding:'3px 6px',background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--text)',borderRadius:'4px',cursor:'pointer'}});
  const sortOptions=[
  ...
  searchInput.addEventListener('input',()=>{
    ...
    const loadingMsg=el('div',{style:{padding:'12px',color:'var(--text-dim)',fontSize:'0.82rem',fontFamily:'var(--font-mono)',textAlign:'center'}},'⟳ loading all threads...');
...
function buildThreadCard(thread){
  ...
  const titleRow=el('div',{style:{display:'flex',alignItems:'center',gap:'6px',marginBottom:'4px'}});
  ...
  meta.appendChild(el('span',{className:'dot',textContent:'·'}));
  meta.appendChild(el('span',{className:'list-comment-likes',style:{fontSize:'0.75rem',color:'var(--text-muted)',fontFamily:'var(--font-mono)'},textContent:'💬 '+(thread.commentCount||0)}));
      ...
      const detailPromises=threadIds.map(threadId=>{
        ...
        return apiCall('thread?id='+threadId,'GET')
          ...
          .then(thread=>{
            ...
            const sortSelect=document.querySelector('#thread-list-container select');
            applySidebarFilter(itemsContainer,searchInput?searchInput.value.trim().toLowerCase():'',sortSelect?sortSelect.value:'newest');
          });
      });
          ...
          const observer=new IntersectionObserver((entries)=>{
            if (entries[0].isIntersecting){
              ...
              const loader=el('div',{
                ...
                style:{padding:'15px',textAlign:'center',fontSize:'0.9rem',color:'#666'},
...
function showThread(threadId,silent,pushHistory){
  if (!threadId){
    console.trace('showThread called with invalid id:',threadId);
  ...
  document.querySelectorAll('.list-thread-container').forEach(el=>{
  ...
  const spinner=el('div',{style:{display:'flex',justifyContent:'center',padding:'40px'}});
  spinner.appendChild(el('div',{className:'spinner'}));
    ...
    .catch(err=>{
      ...
      if (!navigator.onLine){
        try{
          ...
          if (cached){
            const offlineBanner=el('div',{className:'locked-notice',style:{marginBottom:'16px'}},'⚠️ You are offline. Showing cached thread.');
...
function renderThread(container,thread){
  ...
  const titleRow=el('div',{style:{display:'flex',alignItems:'center',gap:'10px',marginBottom:'4px',flexWrap:'wrap'}});
  titleRow.appendChild(el('h1',{id:'thread-title',textContent:thread.title}));
  if (!thread.isPublic) titleRow.appendChild(el('span',{className:'private-badge',textContent:'private'}));
  if (isLocked(thread)){
    const lock=el('div',{className:'locked-notice'});
    lock.appendChild(document.createTextNode('🔒 This thread is locked'));
  ...
  metaRow.appendChild(el('span',{style:{fontSize:'0.82rem',color:'var(--text-dim)',fontFamily:'var(--font-mono)'},textContent:formatDate(thread.createdAt)}));
  wrapper.appendChild(metaRow);

  const bodyDiv=el('div',{id:'thread-body',textContent:thread.content});
...
function loadComments(thread,container){
  ...
  apiCall('comments?threadId='+thread.id,'GET')
    ...
    .catch(()=>{container.appendChild(el('p',{style:{color:'var(--text-dim)'}},'Could not load comments.'));});
}
...
function buildCommentEl(comment,allComments,thread,depth){
  ...
  const authorEl=el('span',{className:'list-comment-author',textContent:'User #'+comment.creatorId});
  ...
  const likeBtn=el('button',{className:'thread-like-button'+(isLikedComment?' liked':''),textContent:isLikedComment?'unlike':'like'});
...
function showReplyModal(parentComment,thread,allComments,commentEl){
  ...
  const submitBtn=el('button',{id:'thread-reply-submit',className:'btn-primary',style:{marginTop:'10px'},textContent:'Comment'});
  submitBtn.addEventListener('click',()=>{
    const content=textarea.value.trim();
...
function showEditCommentModal(comment,thread){
  ...
  const submitBtn=el('button',{id:'thread-edit-submit',className:'btn-primary',style:{marginTop:'10px'},textContent:'Save'});
  submitBtn.addEventListener('click',()=>{
    const content=textarea.value.trim();
    if (!content){showError('Comment cannot be empty.');return;}
...
function showProfile(userId,pushHistory){
  ...
  if (!contentArea){
    loadDashboard();
    setTimeout(()=>showProfile(userId),400);
    return;
  }
  while (contentArea.firstChild) contentArea.removeChild(contentArea.firstChild);
...
function renderProfile(container,user,userId){
  ...
  const avatarImg=el('img',{className:'profile-avatar',src:user.image||generateAvatar(user.name),alt:user.name});
  ...
  if (user.admin) info.appendChild(el('span',{className:'private-badge',style:{marginTop:'6px',display:'inline-block'},textContent:'admin'}));
  profileHeader.appendChild(info);
  wrapper.appendChild(profileHeader);

    if (isSelf){
    const editSection=el('div',{style:{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'20px',marginBottom:'20px'}});
    editSection.appendChild(el('h3',{style:{fontFamily:'var(--font-display)',marginBottom:'16px',fontSize:'1rem'}},'Edit Profile'));
    ...
    const imgInput=el('input',{type:'file',accept:'image/*',style:{background:'transparent',border:'none',padding:'4px 0',color:'var(--text-muted)'}});
    ...
    if (isAdmin&&!isSelf){
    const adminSection=el('div',{style:{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'20px',marginBottom:'20px'}});
    adminSection.appendChild(el('h3',{style:{fontFamily:'var(--font-display)',marginBottom:'12px',fontSize:'1rem'}},'Permissions'));
  ...
  wrapper.appendChild(el('h3',{style:{fontFamily:'var(--font-display)',marginBottom:'12px',fontSize:'1.1rem'}},'Threads by '+user.name));
  ...
  fetchAllUserThreads(0,[])
    .then(allThreadIds=>{
      if (allThreadIds.length===0){
        threadList.appendChild(el('p',{style:{color:'var(--text-dim)',fontSize:'0.9rem'}},'No threads yet.'));
        return;
      }
